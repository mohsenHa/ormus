...
const channelNotFound = "channel not found: %v"

func New(done <-chan bool, wg *sync.WaitGroup, config dconfig.RabbitmqConnection) *ChannelAdapter {
...
func (ca *ChannelAdapter) GetInputChannel(name string) (chan<- []byte, error) {
	...
	return nil, fmt.Errorf(channelNotFound, name)
}
...
func (ca *ChannelAdapter) GetMode(name string) (channel.Mode, error) {
	...
	return "", fmt.Errorf(channelNotFound, name)
}
